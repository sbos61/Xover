...
#   --------------------------------------- # mail send config
	...
	attr_accessor :brwsrType, :brwsrProfile, :testMode, :hlmode, :clearCookie, :headless, :res
...
# puts "var "+var+" value "+ value
				...
				when 'clearCookie'	then @clearCookie= SetConfFlag( value, 'Cookie clearing : ')
...
def XOstartUp(file, mode)
	...
	if !($gcfd.testMode)														# not in test mode
		...
		$brws= GenBrowser.new( $gcfd.brwsrType, $gcfd.brwsrProfile, $gcfd.runMode, $gcfd.pageTimeOut, $gcfd.logPath, $gcfd.clearCookie)
		if $brws.status!=OK
			if($gcfd.hlmode== true)
